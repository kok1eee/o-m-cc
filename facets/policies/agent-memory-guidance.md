...
> 旧 `code-reviewer` agent は v0.58.0 で削除（当時は built-in `Skill: simplify` で置換、v2.1.147 で `Skill: code-review` にリネーム + bug 検出特化、cleanup-and-fix 削除）。コードレビュー観点のクロスリードは不要。
