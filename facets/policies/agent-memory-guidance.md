...
> 旧 `code-reviewer` agent は v0.58.0 で削除（当時は built-in `Skill: simplify` で置換、v2.1.147 で `Skill: code-review` にリネーム + bug 検出特化で cleanup 削除 → v2.1.152 で `--fix` として apply 復活、`/simplify` は alias）。コードレビュー観点のクロスリードは不要。
