# Task Quality Reference

> このファイルは `agents/planner.md` から段階的開示（Progressive Disclosure）で分離されたリファレンスです。
> エージェント実行時に `Read` して適用してください。

## TaskCreate テンプレート

各タスクを `TaskCreate` で登録する。`plan/tasks.md` は使わない。

### subject 形式

`N-M: タスク名`（N=フェーズ番号, M=タスク番号）

### description 形式

```
**What**: [何をするか - 具体的なアクション]
**Where**: [対象ファイル・場所]
**Boundary**: [触らない領域 - 例: 『src/legacy/ は別 PR、既存動作を維持』。該当なしなら省略可]
**How**: [実装アプローチ・使用する技術・パターン]
**Why**: [目的・対応要件 FR-X・なぜこのタスクが必要か]
**Verify**: [検証手順 - コマンド、期待する出力、確認方法]
**見積**: S
**依存**: なし（または N-M）
```

**Boundary の使い所**（cc-sdd の `_Boundary:_` 概念を参考）:
- 既存の周辺コードが「ついで修正」でスコープクリープしがちな領域
- 別 PR / 別スキルで扱う予定の領域（ui-polish 対象の CSS 等）
- 意図的に既存動作を維持したい領域
- design.md の「File Structure Plan > 触らない」から引用できる

### 登録順序

1. Phase 1（基盤構築）のタスクを全て TaskCreate
2. Phase 2（機能実装）のタスクを全て TaskCreate
3. Phase 3（テスト・仕上げ）のタスクを全て TaskCreate
4. 各タスクの依存関係を TaskUpdate の `addBlockedBy` で設定

### チェックリスト（登録前に確認）
- 全ての FR がタスクでカバーされている
- 依存関係に矛盾がない
- テストタスクが含まれている
- 各タスクに Verify（検証手順）が明記されている

## 見積もり基準

| サイズ | 目安 | 例 | 使用方針 |
|--------|------|-----|----------|
| S | 5-30分 | 型定義、設定ファイル、小さな関数 | **推奨** - 基本はこのサイズ |
| M | 30分-1時間 | コンポーネント実装、API実装 | S に分割できない場合のみ |
| L | 1時間以上 | 複雑なロジック、大規模リファクタ | **非推奨** - 必ず S/M に分割 |

**原則:** L サイズのタスクは作らない。分割できないなら設計を見直す。

## タスク品質基準

各タスクは以下の基準を満たすこと:

| 基準 | 説明 |
|------|------|
| **Specific** | 明確なアクション動詞、具体的なファイルパス・関数名・入出力 |
| **Achievable** | 外部ブロッカーなし、実行に必要な情報が完備 |
| **Small** | S サイズ（5-30分）を基本に、M は分割不可の場合のみ。L は禁止 |

## Bite-Sized Steps — How の書き方

**How は「1ステップ = 1アクション」に分解する。**

各ステップは以下のいずれか1つだけ：
- テストを書く
- テストが失敗することを確認する
- 最小限のコードを書いて通す
- テストが通ることを確認する
- コミットする

### 良い例

```markdown
- [ ] 2-1: UserService の作成
  - **What**: ユーザー取得ロジックの実装
  - **Where**: `src/services/UserService.ts`
  - **How**:
    1. `tests/services/UserService.test.ts` に `getById` のテストを書く
    2. テスト実行 → 失敗を確認（UserService が存在しない）
    3. `src/services/UserService.ts` に `getById` を実装
    4. テスト実行 → 通過を確認
    5. コミット: `feat: add UserService.getById`
  - **Why**: FR-1 ユーザー情報の取得
  - **Verify**: `npm test -- UserService` → 0 failures
  - **見積**: S
```

### 悪い例

```markdown
- [ ] 2-1: UserService の作成
  - **How**: UserService を実装してテストを書く
```

→ 何をどの順で行うか不明。テスト先行かどうかも不明。

**アンチパターン:**
- 「実装する」だけで How がない
- ファイルパスが曖昧（「適切な場所に」）
- Verify がない・曖昧（「動作確認」だけ）
- 1タスクで複数の責任を持つ
- How が1行（ステップに分解されていない）
