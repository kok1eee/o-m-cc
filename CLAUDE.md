...
| **Layer 3**: diff ベース品質強制（stop-guard.sh） | 変更行数が閾値超過時に quality-gate を強制 | なし | ハードブロック（唯一の強制停止） |

Layer 1・2 は品質が崩れた時だけ人間の判断を仰ぐ安全弁。Layer 3 は LLM 非依存で品質ゲートを強制する。ゲートは最小限 — チェックポイントが多すぎると「止まらない」価値が薄れる。
