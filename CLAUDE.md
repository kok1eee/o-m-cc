...
| **Layer 3**: diff ベース品質ガード（stop-guard.sh） | 変更行数に応じて quality-gate を推奨/強制 | なし | 500行〜: 推奨（メッセージのみ）、1500行〜: ハードブロック |

Layer 1・2 は品質が崩れた時だけ人間の判断を仰ぐ安全弁。Layer 3 は LLM 非依存の段階的ガード — 500行以上で推奨メッセージ、1500行以上で強制ブロック。普段の作業では柔軟に、大規模変更では確実に品質ゲートを通す。
