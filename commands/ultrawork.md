...
並列エージェントオーケストレーションでタスクを最速で完了します。
...
**AskUserQuestion** でコンテキスト状態を確認：

```
質問: Ultrawork開始前にコンテキストをクリアしますか？

選択肢:
1. /compact を実行（推奨） - 会話を要約してコンテキスト確保
2. /clear を実行 - 完全にクリア（必要な情報は再収集）
3. そのまま続行 - コンテキストに余裕がある場合
```

**推奨**: 大量の並列エージェントを起動するため、事前に /compact でコンテキストを確保。
